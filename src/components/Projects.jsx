    ...
    <section id="projets" className="relative z-10 px-[6%] py-[100px]">
      ...
      <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5.5">
        {projects.map((project) => (
          <div
            key={project.title}
            className={`card-glow-hover relative rounded-[10px] border bg-bg-card p-6.5 transition-all hover:-translate-y-[3px] ${
              project.featured
                ? "border-direction hover:border-direction"
                : "border-border hover:border-socle"
            }`}
          >
            {project.featured && (
